...
import com.infosys.lex.ext.ServiceRepository.bodhi.UserSharedGoalsRepositoryCustom;
import com.infosys.lex.ext.serviceImpl.UserDataUtilImpl;
import com.infosys.lex.ext.util.LexConstants;

@Repository
public class UserSharedGoalsRepositoryImpl implements UserSharedGoalsRepositoryCustom {
	@Autowired
	...
	public List<Map<String, Object>> getUserSharedGoalsByEmail(String userEmail, String goalType, Set<String> goalIds) {
		...
		try {
			...
			Where select = QueryBuilder.select("goal_id", "goal_title", "goal_desc", "goal_start_date", "goal_end_date")
					.from("user_shared_goals").where(QueryBuilder.eq("shared_with", userEmail))
	...
	public Map<String, Object> getUserSharedGoals(String rootOrg, Integer pageSize, String pagingSequence,
		...
		try {
			Select select = QueryBuilder.select().all().from("user_shared_goals");
