export const TEMPLATE_TYPES = {
  ...
  set8:
    // tslint:disable-next-line: max-line-length
    '<div style="width:100%;height: 100%"><a href="{{link}}" target={{target}}><div style="width:100%;height: 100%; display: flex;flex-direction: column;" class="h-full mat-app-background"><img style="width:100%;height: 45%;" src="{{imageSrc}}"/><div style="height: 55%;" class="justify-start m-3"><div class="ws-mat-primary-text leading-tight mat-title" style="line-height: 1.25">{{title}}</div><p class="m-2 mt-6">{{description}}</p></div></div></a></div>',
  set9:
    // tslint:disable-next-line: max-line-length
  ...
  set10:
    // tslint:disable-next-line: max-line-length
    '<div style="width:100%;height: 100%"><a href={{link}} target={{target}}><div class= "relative" style= "background-size: 100% 100%;background-repeat: no-repeat;background-image:url({{imageSrc}});height:100%;width:100%"><div style= "height:auto;background-color: #297595;opacity:0.9" class= "w-full flex flex-col items-center justify-center absolute bottom-0 text-white text-xl font-semibold text-center leading-tight"><div class="text-2xl p-2">{{title}}</div><div class="p-2">{{description}}</div></div></div></a></div>',
  set11:
    // tslint:disable-next-line: max-line-length
  ...
  set12:
    // tslint:disable-next-line: max-line-length
    '<div class="h-full mat-app-background ml-1 mr-4" style="width:100%;height: 100%; display: flex;flex-direction: column;"><a class="flex flex-col" target="{{target}}" href = "{{link}}"><img style="height: 45%; width:100%;" src = "{{imageSrc}}" /><div class="justify-start m-3" style = "height: 55%;" ><div class="leading-tight " style = "line-height: 1.25">{{ category }}</div><p class="m-2 ws-mat-primary-text mat-title mt-6">{{ title }}</p><div ><p>{{ description }}</p></div></div></a></div>',
