...
export class GeneralDetailsComponent implements OnInit {
  ...
  qType = 'My Questions'
  quType = ['My Questions', 'Public Questions']
  expandedElement: any
  groupqueCount = 0
    ...
    if (value && value.contentIdAtSource) {
      ...
      this._service.contestToDraft(idData).subscribe(response => {
        ...
        this._service.getContestDetails(data).subscribe(res => {
          ...
          this.artifactUrl = value.artifactUrl
          const idData2 = {
            _id: this._id,
            artifactUrl: this.artifactUrl,
          }
          this.id.emit(idData2)
  ...
  setQType(value: string) {
    if (value === 'Public Questions') {
      this.questionDetails.myQuestion = 'public'
      this.qType = value
    } else {
      this.questionDetails.myQuestion = 'yes'
      this.qType = value
    }
